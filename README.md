...
程序运行时会注册以下全局快捷键，需按住键盘右侧的 Alt 与 Shift；左侧 Alt/Shift 组合不会触发：

| 快捷键 | 功能 |
| --- | --- |
| `右 Alt + 右 Shift + 左方向键` | 当前枪械基础灵敏度 X −0.01 |
| `右 Alt + 右 Shift + 右方向键` | 当前枪械基础灵敏度 X +0.01 |
| `右 Alt + 右 Shift + 下方向键` | 当前枪械基础灵敏度 Y −0.01 |
| `右 Alt + 右 Shift + 上方向键` | 当前枪械基础灵敏度 Y +0.01 |
| `右 Alt + 右 Shift + -` | 当前枪械基础灵敏度 Y −0.05 |
| `右 Alt + 右 Shift + =` | 当前枪械基础灵敏度 Y +0.05 |
| `右 Alt + 右 Shift + M` | 打开或关闭桌面枪械快捷菜单 |

灵敏度快捷键和托盘灵敏度调整只修改当前选中枪械的基础 X/Y 值，不修改增幅值；连续调整会合并为一次写入，停止操作约 2 秒后自动保存到灵敏度配置文件。快捷键被其他程序占用时，托盘区会提示注册失败。

托盘菜单第一层的“当前灵敏度”行可展开，分别提供当前枪械 X/Y 的 `−0.05`、`+0.05` 调整；连续调整时菜单保持打开。
